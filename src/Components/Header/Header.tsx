...
return (location.pathname != "/signup" && location.pathname != "/login") ? <div data-aos="zoom-out" className="w-full bg-[#000000] px-6 text-white h-20 flex justify-between items-center font-['poppins']">
        <div onClick={() => navigate("/")} className="flex gap-1 cursor-pointer items-center text-bright-sun-400">
            <IconBriefcase
  className="xs-mx:hidden text-3xl font-semibold h-8 w-8 text-orange-500" stroke={2.5}/>
            <div className="xs-mx:hidden text-3xl font-semibold bg-gradient-to-r from-orange-500 via-white to-green-500 bg-clip-text text-transparent">
  VrittiSetu
</div>
        </div>
        {NavLinks()}
